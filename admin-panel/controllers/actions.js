...
class crudDbController {
 ...
 // Find category by id ...
 async readActionsId(req, res) {
  try {
   const resultDb = await readActionId(req.params.id);
   if (resultDb.length == 0)
 ...
 // Update ID card ...
 async updateActionId(req, res) {
  try {
   const resultDb = await  updateAction(req.params.id, req.body);
   if (!resultDb)
 ...
 // Delete ID card.
 async deleteActionId(req, res) {
  try {
   const resultDb = await deleteAction(req.params.id);
   if (!resultDb)
